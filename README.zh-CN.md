...
`monitor` 是常驻、进程内的观测视图。第一次成功的累计 `ccusage` 快照只建立基线；此后每个已接受快照只与前一个已接受快照组成采样对。当前值只在形成有效采样对时更新一次，并在两次采样之间保持不变。总量与模型使用该采样对的 Token 差值除以单调时钟的实际间隔得到 TPM；Agent 与项目保留 Token 单位，展示该采样对的 Token 增长。因此 Monitor 在启动前没有任何读数，也不能重建过去 24 小时图表。进程内原生历史保留采样区间和最长 24 小时的分钟汇总；Timeline 显示窗口默认为 1 小时，可在 5 分钟到 24 小时之间调整。`--window` 只控制当前进程保留的历史，不改变当前值的计算方式。真实监控的 `--interval` 默认 15 秒。Demo Monitor 未显式指定 `--interval` 时以 1 秒的合成节奏推进；隐藏的 `--query-timeout` 限制单个 `ccusage` 子进程的最大时长。
