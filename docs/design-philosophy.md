...
1. **Layout defines topology; Weights describe relative size within the current topology only.**
   ...
   `auto` derives a stable grid from Pane count. A positive `ROWSxCOLUMNS` value fixes the column count while allowing trailing empty cells. `spotlight-wide` gives Pane 0 a full-width top band; `spotlight-wide2` gives Pane 0 and Pane 1 consecutive full-width top bands, then places the remaining Panes in two columns. Those index meanings remain stable as later Panes append in ordinary two-column rows.

   A positional Preset is a launch template: it supplies an initial ordered Pane list, Dashboard defaults, and a layout association; repeated `--pane` fragments append to that list. `--grid ROWSxCOLUMNS` defines a numeric rectangular layout for an explicit Pane list. `--layout auto|spotlight-*` defines a named geometry-only layout for an explicit Pane list. A positional Preset, `--grid`, and `--layout` are mutually exclusive, so the source of initial content and geometry is never ambiguous. `wide`, `narrow`, and `all` are rectangular shortcuts; `auto` is the dynamic rectangular shortcut; Spotlight layouts are named non-rectangular topologies. Runtime `z` selects the same geometry without replacing the live Pane objects, while runtime `Z` creates an explicit numeric-grid draft. Both retain Pane identity, accepted state, focus, configuration, Monitor history, and lifecycle state; changing geometry resets only the logical Weights.

   Full-command copying expands the current Pane list and emits `--grid` or public `--layout`, never the source Preset, so it reproduces the actual session rather than reinserting a template. Changing the configured Layout rebuilds equal Weights for the new topology; no hidden proportions are retained for other Layouts and no terminal pixels are reverse-engineered. Commands record the exact positive integer Weights applicable to the configured Layout, never terminal-dependent absolute dimensions.

2. **Panes share column proportions and per-row heights.**

   A width adjustment changes the selected Pane's shared logical column or Spotlight band; a height adjustment changes its logical row or band. `{`/`}` adjust width, `J`/`K` adjust height, and `+`/`-` remain chart Top controls. A low-total Weight vector expands deterministically to a minimum 24-share pool before adjustment; a larger explicit vector remains exact. Each adjustment then transfers exactly one share against the next logical band, using the preceding band at an edge, without reducing the active integers. Copied commands reproduce those exact positive integer Weights. A spanning Spotlight Pane adjusts through its leading band and remains a span across every covered band; it has no private size. Reordering swaps content without changing geometry or Weights; Pane list index 0 alone determines Spotlight identity, independently of interaction focus. In a fixed grid, Pane insertion preserves columns and appends only as many rows as required; deletion removes only wholly empty trailing rows. Arbitrary coordinates, overlap, per-cell topology, pagination, and Pane scrolling are not supported.
