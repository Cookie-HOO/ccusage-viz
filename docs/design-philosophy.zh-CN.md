...
6. **项目名称是安全的展示投影，不是身份。**
   ...
   Timeline 和 Ranking 只将 Claude 不透明 ID 视为连字符分隔的 token。在完整筛选后项目池中、Top 或 Other 选择可见条目之前，它们会在 token 前缀树的每个分支中消除可证明共享的最深前缀。token 没有路径或质量语义；后缀只须非空且无歧义。无法证明共享前缀的 ID 显示完整不透明标识符，绝不将其解码为文件系统路径。Timeline 和 Ranking 可以更新进程内缓存；缓存只保留这种已证明消除实际产生过的最短安全后缀。Monitor 可以读取缓存，但绝不写入。这些规则绝不改变原始项目身份、Token 归因、选择器成员、聚合键或 Claude–Codex 配对。

   所有按项目图表默认使用 Name 聚合：仅保守合并已证明的 Claude–Codex 配对，并显示不带 Agent 的名称。Exact 聚合保留独立来源项目，且始终显示其 Agent。Monitor 在采样阶段保留精确计数器和增量，仅在其后应用两种展示分组之一。

7. **检查视图通常反映渲染后的图表模型，并提供有界的归因信息。**

   Markdown 和 JSON 数据视图会用 `display_project` 序列化当前显示的项目名称，并在所有项目视图中提供 `merge_group`。非空 `merge_group` 是对真实跨 Agent Name 合并的确定性、仅当前 payload 有效的说明；Exact 或未合并的行没有该组。项目 Ranking 还会将显示出的跨 Agent 聚合展开为安全的来源贡献行，便于核对成员和计算。该机制严格限于解释自动聚合；不得暴露原始记录、路径，也不能演变成通用导出入口。
