...
5. **历史 Ranking 与 Monitor Ranking 只共享视觉语法。**
   ...
   历史 Ranking 对日期范围内累计的 Token 消耗排序。Monitor Ranking 对实时观察窗口中的数值排序：Model 可以表达 TPM，Agent 和 Project 表达窗口内 Token 增长。两者不共享时间或指标契约。

## 数据一致性与异步结果

1. **`??` 只表示所需数据尚未取得。**

   `??` 是统一的加载中占位，表示当前状态所需的数据缺失且已经进入查询流程，包括防抖等待、待执行或执行中。不得用它表达真实零值、无数据、错误、不可用或不适用；这些状态必须各自明确呈现。Coverage 尚未完成时，也不得用旧值或其他候选配置的结果代替未知数据。只有已覆盖区间内缺失的记录才可以解释为真实零值。
...
5. **持续 TUI 的普通设置即时生效。**
   ...
   普通设置界面中的 Enter 与 Escape 都保留当前有效状态并返回。Layout 和 Weights 同样即时生效。

   **例外：**拥有未提交草稿的嵌套编辑器使用 Enter 确认、Escape 放弃，例如 Filter Editor 和尚未创建的 Pane。

6. **按键表达稳定概念，而不是通用大小写规则。**
...
10. **检查视图先展示内容，再允许复制。**
   ...
   **例外：**Dashboard 全局只提供紧凑命令和完整命令，不提供跨 Pane 的 Markdown 或 JSON；单个 Pane 通过全屏 Inspector 提供对应 Standalone 内容。
...
2. **Pane 共享列比例与各行高度。**
   ...
   调整列比例影响共享列，调整行高度影响对应行。Pane 重排只交换内容，不改变几何。不支持任意坐标、重叠、每格独立拓扑、分页或 Pane 滚动。
...
5. **Execution Coordinator 减少物理工作，但不合并业务状态。**
   ...
   **例外：**Coordinator 当前不合并不同日期范围或 Filters，不构造 Provider 字段超集，不复用已完成的过时结果，也不延迟 Tick 收集批次。未来 Provider 插件只有显式提供合并与拆分契约时，才能执行更高级优化；Interval 语义始终位于 Coordinator 之外。
...
7. **刷新包含结果所需的重新计算与重绘。**
   ...
   **例外：**Pause 只停止周期 Tick；暂停期间不产生周期工作或积累 backlog，但手动刷新和用户修改配置后经防抖合并的一次性补齐仍可执行。此类补齐不会恢复周期调度，Monitor 的暂停区间仍按 Gap 处理。Standalone Watch 与 Monitor 保留自己的刷新、暂停和 Interval 行为。
