...
5. **除非有效取值必须依赖草稿，否则配置即时生效。**
   ...
   **例外：**只有必须完成输入或选择后才能形成有效取值的操作使用隔离草稿，例如 Filter Editor 和尚未创建的 Pane。多选草稿使用 Space 切换选择、Enter 确认完整草稿、Escape 放弃；确认前不得改变有效配置、generation 或当前工作。只有操作具有破坏性、难以恢复或后果不够清晰时才增加二次确认，普通可逆配置不需要确认。
