...
3. **能力通过 Quick 和 Advanced 渐进呈现。**
   ...
   Standalone、Pane 与 Dashboard 全局调整统一只使用两行：第一行展示运行状态与当前生效设置，第二行展示当前 Quick 或 Advanced 操作。`a` 切换页面；普通调整中 Enter 与 Escape 都保留已生效修改并退出。不提供可点击的“完成”。窄终端按完整操作单元隐藏低优先级项，并用 `…(+N)` 精确提示仍可通过键盘使用的操作数量；页面身份、`a` 切换与 Enter/Escape 提示始终保留。
...
11. **检查视图先展示内容，再允许复制。**
   ...
   **例外：**Dashboard 全局只提供紧凑命令和完整命令，不提供跨 Pane 的 Markdown 或 JSON。每个 Pane 独立拥有自己的 `v` 视图循环：Chart、Command、Full Command、数据表和数据 JSON。Dashboard 中按 `c` 会保留当前画面并临时关闭鼠标上报，让终端接管拖选与复制；Escape 或任意键盘输入退出选择模式，即使发生中断也会恢复鼠标上报。Standalone 不启用 Dashboard 鼠标上报，因此不需要该模式。
