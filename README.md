...
Dashboard browse mode keeps one concise control row: `r` refreshes all, `s`/click adjusts a Pane, `g` opens global adjustment, `h` hides or shows controls, `v` shows the full command, and Space pauses or resumes scheduling. Pane adjustment starts with the same state and Quick/Advanced chart rows as Standalone, then adds a divider and two Dashboard-management rows. `a` switches only the chart action row; management remains visible and active on either page. Fixed layouts preserve their column count and add rows when `N`, `n`, or Global `+` needs capacity.
