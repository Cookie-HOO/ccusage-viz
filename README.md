...
`monitor` is an always-running, process-local observation. The first successful cumulative `ccusage` snapshot establishes a baseline; each later accepted snapshot is paired only with the preceding accepted snapshot. The current value changes once per valid pair and remains unchanged between samples. Total and Model divide that pair's Token delta by its actual monotonic elapsed time to produce TPM; Agent and Project retain Token units and show that pair's Token delta. Monitor therefore has no readings before startup and cannot reconstruct a past 24-hour chart. Native process-local history keeps intervals and minute rollups for up to 24 hours; the displayed Timeline window defaults to one hour and can be adjusted from 5 minutes to 24 hours. `--window` controls only this invocation's retained history (5m–24h, default `1h`); it does not change how the current value is calculated. `--interval` is the target delay between sampling attempts (default 15 seconds for real monitoring). Demo Monitor defaults to a 1-second synthetic cadence unless `--interval` is explicitly supplied; hidden `--query-timeout` bounds one `ccusage` subprocess invocation.

Omitting `--by` shows authoritative **Total TPM**. `--by model` shows per-model TPM; `--by agent` and `--by project` show the latest valid pair's Token growth. Grouped views can use `--legend values` for a minimal, marker-free list of each visible item and its latest displayed observation. Compact Monitor ranking uses an arrow beside the rank only for real rank movement and `↑`, `↓`, or `—` beside the value for increase, decrease, or equality (`^`, `v`, and `=` with `--ascii`); it intentionally has no activity point because every Monitor series is live. The initial baseline is marker-free, while a series first seen later gets value-up but no fabricated rank arrow. Grouped Monitor modes default to Top 3 and fold the remainder into `Other`. `--agent`, `--model`, and `--project` are startup-only source filters. Monitor exits with `Ctrl-C`; `r` samples now, Space pauses/resumes, and `v` cycles chart → compact command → full command → Markdown data table → JSON data → chart. The compact command omits defaults; full command makes every effective setting explicit. Chart view permits `m` adjustment and has no copy shortcut. Every other view permits `y` copy but not adjustment. The table and JSON serialize retained display buckets plus the separate current observation, including grouping, Top N, and `Other`, never raw counters. `h` hides or restores the session-only control footer to reclaim chart rows, and `m` opens the two-row runtime adjustment panel only from chart view. Its Quick page adjusts window, interval, grouping, Top, theme, and style without querying again or losing retained history; `a` switches to Advanced, where legend placement lives, and `y` copies the candidate command. Startup `--model` filters are preserved and are not runtime controls. Adjustment guides always remain visible and closing the panel restores the prior footer preference. Monitor x-axis labels use `HH:MM`. Use `--theme` and `--style` to choose an initial appearance; use `monitor --demo` for immediate style exploration because real observed history does not exist at startup. Demo monitor data is deterministic, starts with a fluctuating in-memory history, stays in memory, and never invokes `ccusage`.

Observed TPM is neither QPM nor API rate-limit TPM. There is no QPM metric. External QPM, when added in a future release, will count logical requests and will never infer requests from tokens or retries. Monitor `style=ranking` is a compact latest-pair view, not the cumulative historical `ranking` command. Monitor-only `style=list` uses that same observed ranking model but omits its bar track: it centers a compact table while retaining ordinal, rank movement, activity, value, and value-trend markers. Historical `ranking` does not expose `list`. Establishing a baseline, detecting a sleep or sampling gap, pausing or resuming, changing data-affecting configuration, or detecting a counter reset clears the current value until the next valid pair; retained Timeline history remains available with discontinuities represented as gaps. Monitor does not claim historical per-minute data, arbitrary date-range hourly distribution, or a rolling period that predates process startup. Sampling gaps, query errors, and counter decreases are not treated as zero traffic.
...
`dashboard` owns one terminal input loop and compositor while each Pane keeps its latest result, failure state, and—when it is a Monitor Pane—its own in-memory observation history. It starts with a compact loading state rather than an empty framed grid. The default `wide` preset is a filled 2×2 overview: Timeline, Stack, Ranking, and Monitor grouped by Model. Bare `ccuv dashboard` is equivalent to `ccuv dashboard wide`; `narrow` provides a three-pane vertical view, `all` expands all ten representative views into a framed 5×2 grid, `spotlight-wide` features Timeline across the top, and `spotlight-tall` features Ranking down the left. A preset supplies the base panes and settings, explicit Dashboard options override it, and repeated `--pane` values append. Without a preset, `--pane` defines the complete list. The generated Wide Monitor is equivalent to `monitor --by model`; standalone `monitor` and an explicit `--pane "monitor"` still show authoritative Total TPM. `--pane` takes a quoted chart fragment beginning with `timeline`, `calendar`, `stack`, `ranking`, or `monitor`; Host, process, and lifecycle options are rejected inside fragments. Dashboard owns cadence: `--refresh-interval` refreshes Historical Panes and `--sampling-interval` samples Monitor Panes. `--grid` accepts topology only: `auto`, `ROWSxCOLUMNS`, `spotlight-wide`, `spotlight-wide2`, or `spotlight-tall`. The content presets `wide`, `narrow`, and `all` are launch-time templates, not `--grid` values.

In every Spotlight layout, Pane list index 0 is featured: click and `Tab` change interaction focus only, while reorder, insertion before index 0, or deletion of index 0 changes which Pane is featured. `spotlight-wide2` gives Pane 0 and Pane 1 their own full-width leading rows, then places remaining Panes in two columns. `spotlight-tall` renders with a wide vertical fallback on narrow terminals without changing its configured name or saved proportions.
...
Browse mode intentionally has one concise footer row: `r` refresh all, `s`/click adjust a pane, `g` open global adjustment, `h` hide/show controls, `v` show the full command, and Space pause/resume scheduling. Controls are a session shell and remain independent of chart Density. Data, capability, renderer, and supplemental-comparison Notices stay in a bounded band inside their source Pane; they are neither moved into nor deduplicated through a global warning block, and one Pane's Notices do not change another Pane's geometry. Pane adjustment starts with the same two chart rows as Standalone: current effective state, then Quick or Advanced chart actions. A divider and two Dashboard-only rows follow for content/lifecycle actions and position/focus actions. `a` switches only the chart action row; Dashboard management remains visible and active on either page. These actions include View, Replace, `N` insert before, `n` insert after, Delete, move previous/next, and Pane navigation. Fixed layouts preserve their column count and grow only the row count when Pane insertion needs more capacity; deletion reclaims only completely empty trailing rows. Global adjustment is one Dashboard-wide page containing Theme, Style, Header, Summary, and Layout. Pane addition, deletion, ordering, selection, and logical width/height ratio adjustment belong only to selected-Pane management; copied Dashboard commands preserve the normalized row and column weights. Dashboard-global settings do not rewrite Pane-owned Density, Theme, Style, Filters, or analysis settings. A copied Dashboard preserves shell Theme/Style and each Pane’s independent presentation. `Ctrl-C` restores the terminal and cancels active child queries.

Monitor owns an anchored, padded y-axis shared by standalone and Dashboard rendering. The bound expands immediately when data crosses it and shrinks only after sustained lower utilization, so small fluctuations move the line instead of continuously moving the axis. This changes presentation only: Total and Model remain latest-pair TPM, while Agent and Project remain latest-pair Token growth.
...
Pane width and height controls adjust shared logical column and row ratios, not private pixel sizes. A selected band exchanges one share with the next band, or with the preceding band at the edge; when the donor has one share, all weights are temporarily scaled before the transfer so either direction remains responsive. Commands retain the resulting positive, GCD-normalized ratios. A spanning Spotlight Pane uses the leading band as its adjustment anchor and continues to span every covered band; shared auxiliary Panes reveal the changed proportions.
...
Dashboard browse mode keeps one concise control row: `r` refreshes all, `s`/click adjusts a Pane, `g` opens global adjustment, `h` hides or shows controls, `v` shows the full command, and Space pauses or resumes scheduling. Pane adjustment starts with the same state and Quick/Advanced chart rows as Standalone, then adds Dashboard management for Pane lifecycle, ordering, navigation, and logical width/height ratios. `a` switches only the chart action row; management remains visible and active on either page. Global adjustment has one Dashboard-wide page for Theme, Style, Header, Summary, and Layout. Fixed layouts preserve columns, add rows for insertion, and reclaim only completely empty trailing rows after deletion.
